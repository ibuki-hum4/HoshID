...
# root で動かさない。**数値で指定する。** k8s の `runAsNonRoot` は
# イメージの USER を見るが、kubelet はユーザー名を解決できないので
# `USER node` だと「root で動く」と判定されて Pod が起動しない。
# 1000 は node イメージの node ユーザー。
# WORKDIR は root が作るので、先に所有者を移す。
RUN chown 1000:1000 /app
USER 1000:1000

COPY --chown=1000:1000 --from=build /app/.next/standalone ./
COPY --chown=1000:1000 --from=build /app/.next/static ./.next/static
...
COPY --chown=1000:1000 --from=build /app/node_modules/.prisma ./node_modules/.prisma
COPY --chown=1000:1000 --from=build /app/node_modules/@prisma ./node_modules/@prisma
...
# **root で動かさない。** かつ、数値で指定する。k8s の `runAsNonRoot` は
# イメージの USER を見るが、kubelet はユーザー名を解決できないので
# `USER bun` だと「root で動く」と判定されて Pod が起動しない。
# 1000 は oven/bun イメージの bun ユーザー。
RUN chown 1000:1000 /app
USER 1000:1000
...
COPY --chown=1000:1000 --from=build /app/node_modules/.prisma ./node_modules/.prisma
COPY --chown=1000:1000 package.json bun.lock tsconfig.json prisma.config.ts ./
